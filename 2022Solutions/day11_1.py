...
monkeyInspects = [0 for i in range(len(monkeyItems))]

for k in range(20):
    for i in range(len(monkeyItems)):
        for j in range(len(monkeyItems[i])):
            monkeyInspects[i] += 1
            old = monkeyItems[i][j]
            monkeyItems[i][j] = eval(monkeyOperations[i])
            monkeyItems[i][j] = monkeyItems[i][j] // 3
