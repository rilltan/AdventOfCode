...
visited = set()
queue = [(0,0,0,"E",0),(0,0,0,"S",0)] # (distance,row,column,direction,straight line distace)
heapq.heapify(queue)
result = 100000
while queue:
    ...
    if (current[1],current[2],current[3],current[4]) not in visited:
        ...
        for d in u_dirs:
            ...
            if d!=current[3] and current[4]<4:
                continue
            ...
            if r == len(data)-1 and c == len(data[0])-1 and straight>=4:
